...
export default function UploadPage({ onUploaded }) {
  const [files, setFiles] = useState([])          // [{file, category, status, result, error}]
  ...
  function addFiles(newFiles) {
    const entries = Array.from(newFiles).map(f => {
      ...
      let detectedCategory = CATEGORIES[0]; // Default is Education Loan
      if (name.includes('education')) {
        detectedCategory = 'Education Loan';
      } else if (name.includes('home')) {
        detectedCategory = 'Home Loan';
      } else if (name.includes('personal')) {
        detectedCategory = 'Personal Loan';
      } else if (name.includes('vehicle') || name.includes('car') || name.includes('auto')) {
        detectedCategory = 'Vehicle Loan';
      } else if (name.includes('business') || name.includes('sme') || name.includes('corporate')) {
        detectedCategory = 'Business Loan';
      }
      return {
        file: f,
