...
import { FileText, Trash2, RefreshCw, Lock } from 'lucide-react'
...
export default function DocsPage() {
  ...
  const remove = async (id: number) => {
    if (!isEmployee) {
      alert('Document deletion is restricted to Bank Employees.')
      return
    }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Knowledge Base Documents</h1>
          {!isEmployee && (
            <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
              <Lock size={12} className="text-amber-500" /> Read-only mode for Customers. Log in as Bank Employee to manage files.
            </p>
          )}
        </div>
      ...
      <div className="space-y-3">
        {docs.map(d => (
          <div key={d.id} className="flex items-center gap-3 bg-white border rounded-lg px-4 py-3 shadow-sm">
            ...
            {isEmployee ? (
              <button onClick={() => remove(d.id)} title="Delete Document (Employee)" className="text-gray-300 hover:text-red-500 transition-colors p-1">
                <Trash2 size={16} />
              </button>
            ) : (
              <span className="text-xs text-slate-300 font-mono px-2 py-0.5 border border-slate-100 bg-slate-50 rounded">
                Verified
              </span>
            )}
          </div>
