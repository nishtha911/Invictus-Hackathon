...
export function CallWithAiModal({ lead, onClose, onCallLogged }: CallWithAiModalProps) {
  ...
  const handleUserTurn = useCallback(async (text: string) => {
    ...
    const allPriorAssistant = transcriptRef.current.filter((t) => t.role === "assistant").map((t) => t.text);
    const prevAssistant = allPriorAssistant.at(-1) ?? "";
    ...
    try {
      ...
      reply = localTurn(text, ctx, prevAssistant, allPriorAssistant); // slow / offline — keep the conversation moving
    }
